...
export class InMomoryPetsRepository implements PetsRepository {
  ...
  async searchPets({
    ...
    limit,
    ...petFilters
  }: Filters): Promise<{pets: Pet[]; count: number}> {
    ...
    const filterPets = () => {
      ...
      petsByCity.forEach((pet) => {
        if (
          (!petFilters.age_min || pet.age >= petFilters.age_min) &&
          (!petFilters.age_max || pet.age <= petFilters.age_max) &&
          (!petFilters.pet_type || pet.pet_type === petFilters.pet_type) &&
          (!petFilters.sex || pet.sex === petFilters.sex) &&
          (!petFilters.name || pet.name.includes(petFilters.name)) &&
          (!petFilters.breed || (pet.breed !== null && petFilters.breed.includes(pet.breed))) &&
          (!petFilters.size || petFilters.size === pet.size || petFilters.size.includes(pet.size))
        ) {
          filtered.push(pet);
        }
      });
